...
public class MeatHookEntityRenderer extends GeoEntityRenderer<MeatHookEntity> {
    ...
    private static final RenderType CHAIN_LAYER = RenderType.entitySmoothCutout(
    ...
    public void renderChain(MeatHookEntity hookshot, float x, float y, float z, float tickDelta, int tickCount, PoseStack poseStack, MultiBufferSource provider, int light) {
        ...
        if (hookshot.getVariant() == 1) {
            vertexConsumer = provider.getBuffer(PLASMA_LAYER);
        } else {
            vertexConsumer = provider.getBuffer(CHAIN_LAYER);
        }
        var vertX1 = 0F;
        var vertY1 = 0.25F;
        ...
        var minU = 0F;
        var maxU = 0.1875F;
        var minV = 0.0F - (tickCount + tickDelta) * 0.01F;
        var maxV = Mth.sqrt(squaredLength) / 8F - (tickCount + tickDelta) * 0.01F;
