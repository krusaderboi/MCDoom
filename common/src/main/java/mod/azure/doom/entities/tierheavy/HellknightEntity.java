...
public class HellknightEntity extends DemonEntity implements SmartBrainOwner<HellknightEntity> {
    ...
    public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(HellknightEntity.class,
    ...
    public static AttributeSupplier.@NotNull Builder createMobAttributes() {
        return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH,
                MCDoom.config.hellknight_health).add(Attributes.KNOCKBACK_RESISTANCE, 0.6f).add(
                Attributes.ATTACK_DAMAGE, MCDoom.config.hellknight_melee_damage).add(Attributes.MOVEMENT_SPEED,
    ...
    public BrainActivityGroup<HellknightEntity> getIdleTasks() {
        return BrainActivityGroup.idleTasks(new FirstApplicableBehaviour<HellknightEntity>(
    ...
    public BrainActivityGroup<HellknightEntity> getFightTasks() {
        return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf(
                ...
                new DemonProjectileAttack<>(7).attackInterval(mob -> 80).attackDamage(
                        MCDoom.config.hellknight_ranged_damage), new DemonMeleeAttack<>(5));
    }
