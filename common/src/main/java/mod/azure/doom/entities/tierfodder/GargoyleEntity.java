...
public class GargoyleEntity extends DemonEntity implements SmartBrainOwner<GargoyleEntity> {
    ...
    public static AttributeSupplier.@NotNull Builder createMobAttributes() {
        return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.MAX_HEALTH,
                MCDoom.config.gargoyle_health).add(Attributes.ATTACK_DAMAGE, 3.0D).add(Attributes.FLYING_SPEED,
    ...
    public void registerControllers(ControllerRegistrar controllers) {
        ...
        controllers.add(new AnimationController<>(this, "livingController", 0, event -> {
            ...
            return PlayState.CONTINUE;
                ...
                new AnimationController<>(this, "attackController", 0, event -> PlayState.STOP).triggerableAnim("death",
                        DoomAnimationsDefault.DEATH).triggerableAnim("ranged",
                        DoomAnimationsDefault.ATTACKING).triggerableAnim("melee", DoomAnimationsDefault.ATTACKING));
    }
    ...
    public BrainActivityGroup<GargoyleEntity> getIdleTasks() {
        return BrainActivityGroup.idleTasks(new FirstApplicableBehaviour<GargoyleEntity>(
    ...
    public BrainActivityGroup<GargoyleEntity> getFightTasks() {
        return BrainActivityGroup.fightTasks(new InvalidateAttackTarget<>().invalidateIf(
                ...
                new DemonProjectileAttack<>(7).attackInterval(mob -> 80).attackDamage(
                        MCDoom.config.gargoyle_ranged_damage), new DemonMeleeAttack<>(5));
    }
